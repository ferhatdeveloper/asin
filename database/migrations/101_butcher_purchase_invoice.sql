-- 101: Kasap üretim fişine bağlı alış faturası (çift belge engeli)
-- butcher_orders: purchase_invoice_id / no + tedarikçi alanları
-- INIT_BUTCHER_PRODUCTION_TABLES güncellenir (yeni firma + mevcut ALTER)
...
BEGIN
  FOR r IN
    SELECT DISTINCT regexp_replace(tablename, '^rex_([0-9]+)_butcher_orders$', '\1') AS firm_nr
    FROM pg_tables
    WHERE schemaname = 'public'
      AND tablename ~ '^rex_[0-9]+_butcher_orders$'
  LOOP
