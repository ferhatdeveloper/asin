-- 103: Kasap — alış / stok olmadan üretim tamamlanabilsin
-- butcher_settings.allow_complete_without_stock (varsayılan true)
...
BEGIN
  FOR r IN
    ...
    WHERE schemaname = 'public' AND tablename ~ '^rex_[0-9]+_butcher_settings$'
  LOOP
    EXECUTE format(
      'ALTER TABLE public.%I ADD COLUMN IF NOT EXISTS allow_complete_without_stock BOOLEAN NOT NULL DEFAULT true',
      r.tablename
  ...
  END LOOP;
  FOR r IN
    SELECT DISTINCT regexp_replace(tablename, '^rex_([0-9]+)_butcher_settings$', '\1') AS firm_nr
    FROM pg_tables
    WHERE schemaname = 'public' AND tablename ~ '^rex_[0-9]+_butcher_settings$'
  LOOP
