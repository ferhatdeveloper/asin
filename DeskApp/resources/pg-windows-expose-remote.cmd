...
if not "%RETAILEX_ELEV_ARGS%"=="" (
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0pg-windows-expose-remote.ps1" %RETAILEX_ELEV_ARGS%
  set "RETAILEX_ELEV_ARGS="
) else (
  powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0pg-windows-expose-remote.ps1" %*
)
endlocal
exit /b %errorlevel%
